...
const serverlessConfiguration: AWS & { build: { esbuild: boolean } } = {
  ...
  provider: {
    ...
    iamRoleStatements: [
      {
        ...
        Resource: [
          ...
          "arn:aws:dynamodb:${self:provider.region}:${aws:accountId}:table/${self:custom.spriteAssetsTableName}",
          "arn:aws:dynamodb:${self:provider.region}:${aws:accountId}:table/${self:custom.spriteAssetsTableName}/index/GSI-sprite-assets-by-owner",
          ...
          "arn:aws:dynamodb:${self:provider.region}:${aws:accountId}:table/${self:custom.magicLayerJobsTableName}",
          "arn:aws:dynamodb:${self:provider.region}:${aws:accountId}:table/${self:custom.creativeProjectsTableName}",
    ...
    environment: {
      ...
      spriteAssetsTable: "${self:custom.spriteAssetsTableName}",
      ...
      spriteAssetsBucket: "${self:custom.spriteAssetsBucket}",
      ...
      SACACARNET_CONTACT_FROM_EMAIL: "${env:SACACARNET_CONTACT_FROM_EMAIL, 'danielar8268@gmail.com'}",
      SACACARNET_CONTACT_TO_EMAIL: "${env:SACACARNET_CONTACT_TO_EMAIL, 'danielar8268@gmail.com'}",
      ...
      MAGIC_LAYER_WORKER_FUNCTION_NAME: "${self:custom.magicLayerWorkerFunctionName}",
      MAGIC_LAYERS_ACCESS_KEY: "${ssm:${self:custom.magicLayersAccessKeyParameterName}}",
      CREATIVE_STUDIO_ACCESS_KEY: "${ssm:${self:custom.magicLayersAccessKeyParameterName}}",
      CREATIVE_STUDIO_MODEL: "${env:CREATIVE_STUDIO_MODEL, 'gpt-5.6-terra'}",
      ...
      REMATES_MAX_DOCUMENTS_PER_SOURCE_PER_RUN: "${env:REMATES_MAX_DOCUMENTS_PER_SOURCE_PER_RUN, '25'}",
      REMATES_CJ_MAX_DOCUMENTS_PER_RUN: "${env:REMATES_CJ_MAX_DOCUMENTS_PER_RUN, '5'}",
      REMATES_CJ_MAX_PAGES: "${env:REMATES_CJ_MAX_PAGES, '5'}",
      REMATES_DOCUMENT_REFRESH_DAYS: "${env:REMATES_DOCUMENT_REFRESH_DAYS, '28'}",
  ...
  custom: {
    ...
    spriteAssetsTableName: "${sls:stage}-sprite-assets",
    ...
    spriteAssetsBucket: "${sls:stage}-sprite-assets-du-portfolio",
    reelAssetTtlDays: 7,
