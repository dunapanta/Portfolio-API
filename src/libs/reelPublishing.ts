...
import { createDownloadUrl } from "@libs/reelJobs";
import {
...
export type PublishPlatform = "facebook" | "instagram" | "youtube" | "x";
...
export const selectPublishAsset = (assets: ReelAsset[]) => {
...
export const publishYouTubeShort = async ({
  asset,
  connection,
  metadata,
  videoUrl,
...
export const publishXPost = async ({
  asset,
  connection,
  metadata,
  videoUrl,
...
/**
 * Publish one reel to one platform. Shared by the HTTP publish endpoint and
 * the scheduled-publish cron so both paths behave identically.
 */
export const publishReelToPlatform = async ({
  metadata,
  platform,
  reel,
}: {
  ...
  if (!connection) throw new Error(`Connect ${platform} before publishing.`);

  const videoUrl = await createDownloadUrl(asset.key, asset.bucket);
  const publishResults =
    typeof reel.publishResults === "object" && reel.publishResults ? reel.publishResults : {};
  ...
  return platform === "instagram"
    ...
    ? publishFacebookVideo({
        ...
        title: String(metadata.title || "Swipe2Play"),
        videoUrl,
      })
    : platform === "x"
    ? publishXPost({ asset, connection, metadata, videoUrl })
    : publishYouTubeShort({ asset, connection, metadata, videoUrl });
};
